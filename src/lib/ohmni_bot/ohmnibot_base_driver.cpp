#include <edu_robot/ohmni_bot/ohmnibot_base_driver.h>
...
Ohmnibot::Ohmnibot()
    ...
    {
        if(!loadParameter) ROS_ERROR(_logger_prefix <<"failed to load parameters from server");
        ...
        _odometry_component = std::make_shared<processing::OdometryEstimator>(processing::OdometryEstimator::Parameter{},*this);
...
{
    ROS_INFO(_logger_prefix <<"load parameters from server");
    ...
    privateNh.param("sub_twist_topic", _parameter.sub_twist_topic, "cmd_vel");
    privateNh.param("pub_odom_topic", _parameter.pub_odom_topic, "odometry");
...
void Ohmnibot::cbCmdVel(const geometry_msgs::Twist::ConstPtr& twist_msg)
{
  ...
  try {
    ...
    if(ros::Time::now().toSec() > (_stamp_cmd_vel.toSec() + parameter.timeout_cmd_vel))
    {
    ...
    _pub_odometry.publish(_odometry_component->getOdometryMessage(
      getFrameIdPrefix() + parameter.tf_footprint_frame, getFrameIdPrefix() + "odom"
    ));

    if (parameter.publish_tf_odom) {
